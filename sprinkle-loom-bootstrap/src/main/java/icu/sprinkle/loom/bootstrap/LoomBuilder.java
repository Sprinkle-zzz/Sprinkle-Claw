...
import icu.sprinkle.loom.core.loop.*;
import icu.sprinkle.loom.core.loop.ToolOutputTruncator;
import icu.sprinkle.loom.core.observability.AgentMetrics;
import icu.sprinkle.loom.core.observability.AgentTracer;
import icu.sprinkle.loom.core.prompt.SystemPromptBuilder;
import icu.sprinkle.loom.core.session.SessionManager;
import icu.sprinkle.loom.core.session.SessionStore;
import icu.sprinkle.loom.core.snapshot.FileTimestampCache;
import icu.sprinkle.loom.core.snapshot.FileSnapshot;
...
import icu.sprinkle.loom.tool.annotation.AnnotatedToolAdapter;
...
/**
 * Agent 流式构建器，用于组装和配置 {@link Loom} 实例。
 *
 * <h3>组装流程</h3>
 * <ol>
 ...
 * @since 2026/3/20
 */
@icu.sprinkle.loom.api.Stable
public final class LoomBuilder {

    private static final Logger log = LoggerFactory.getLogger(LoomBuilder.class);
     ...
     */
    public LoomBuilder apiKey(String apiKey) {
        this.apiKey = apiKey;
        return this;
    }
     ...
     */
    public LoomBuilder model(String model) {
        this.model = model;
        return this;
    }
     ...
     */
    public LoomBuilder baseUrl(String baseUrl) {
     ...
     */
    public LoomBuilder providerId(String providerId) {
     ...
     * 设置工作目录。
     */
    public LoomBuilder workingDirectory(Path dir) {
     ...
     */
    public LoomBuilder workdir(Path dir) {
     ...
     */
    public LoomBuilder skillsDirectory(Path dir) {
     ...
     */
    public LoomBuilder tasksDirectory(Path dir) {
     ...
     * 设置自定义系统提示词。
     */
    public LoomBuilder systemPrompt(String prompt) {
     ...
     * 设置最大循环迭代次数。
     */
    public LoomBuilder maxIterations(int max) {
     ...
     * 设置循环总超时时间。
     */
    public LoomBuilder loopTimeout(Duration timeout) {
     ...
     * 设置单个工具执行的超时时间。
     */
    public LoomBuilder toolTimeout(Duration timeout) {
     ...
     * 添加一个工具。
     */
    public LoomBuilder addTool(AgentTool tool) {
        this.tools.add(tool);
        return this;
    }
     ...
     * 批量添加工具。
     */
    public LoomBuilder addTools(Collection<? extends AgentTool> tools) {
        this.tools.addAll(tools);
        return this;
    }

    /**
     * 添加包含 {@link icu.sprinkle.loom.tool.annotation.Tool @Tool} 注解方法的对象。
     */
    public LoomBuilder addAnnotatedTools(Object holder) {
     ...
     * 添加循环生命周期钩子。
     */
    public LoomBuilder addHook(LoopHook hook) {
        this.hooks.add(hook);
        return this;
    }
     ...
     * 添加一个被禁止的命令。
     */
    public LoomBuilder blockCommand(String command) {
     ...
     * 批量添加被禁止的命令。
     */
    public LoomBuilder blockCommands(Collection<String> commands) {
     ...
     */
    public LoomBuilder header(String name, String value) {
     ...
     */
    public LoomBuilder llmProvider(LlmProvider provider) {
     ...
     * 设置工具安全策略。
     */
    public LoomBuilder toolPolicy(ToolPolicy policy) {
     ...
     * 设置工具错误处理器。
     */
    public LoomBuilder toolErrorHandler(ToolErrorHandler handler) {
     ...
     * 设置 Agent 循环错误处理器。
     */
    public LoomBuilder agentErrorHandler(AgentErrorHandler handler) {
     ...
     * 设置指标收集器。
     */
    public LoomBuilder metrics(AgentMetrics metrics) {
     ...
     * 设置追踪器。
     */
    public LoomBuilder tracer(AgentTracer tracer) {
        this.tracer = tracer;
        return this;
    }
     ...
     * 设置会话存储后端（启用会话持久化）。
     */
    public LoomBuilder sessionStore(SessionStore store) {
     ...
     */
    public LoomBuilder autoSaveInterval(int interval) {
     ...
     * 设置触发自动压缩的 token 阈值。
     */
    public LoomBuilder compactionThreshold(int threshold) {
     ...
     */
    public LoomBuilder modelContextWindow(int contextWindow) {
     ...
     */
    public LoomBuilder todoNagThreshold(int threshold) {
     ...
     */
    public LoomBuilder enableFileTools() {
     ...
     */
    public LoomBuilder enableBashTool() {
     ...
     */
    public LoomBuilder enableCodingTools() {
        this.enableFileTools = true;
        this.enableBashTool = true;
        this.enableTodoWrite = true;
     ...
     */
    public LoomBuilder enableManualCompact() {
        this.enableManualCompact = true;
        return this;
    }
     ...
     */
    public LoomBuilder enableFileSnapshot() {
        this.enableFileSnapshot = true;
        return this;
    }

    /**
     * 启用子 Agent 派生（需要 sprinkle-loom-agent-ext 在 classpath）。
     */
    public LoomBuilder enableSubAgent() {
        this.enableSubAgent = true;
        return this;
    }

    /**
     * 启用 Skill 两层加载（需要 sprinkle-loom-agent-ext 在 classpath）。
     */
    public LoomBuilder enableSkill() {
        this.enableSkill = true;
        return this;
    }

    /**
     * 启用持久化任务板（需要 sprinkle-loom-agent-ext 在 classpath）。
     */
    public LoomBuilder enableTaskBoard() {
        this.enableTaskBoard = true;
        return this;
    }

    /**
     * 启用后台任务管理（需要 sprinkle-loom-agent-ext 在 classpath）。
     */
    public LoomBuilder enableBackgroundTasks() {
        this.enableBackgroundTasks = true;
        return this;
    }

    /**
     * 设置 Agent 身份描述（压缩后自动重注入，需要 sprinkle-loom-agent-ext 在 classpath）。
     */
    public LoomBuilder identityPrompt(String prompt) {
     ...
     */
    public LoomBuilder addSkill(String name, String description, String body) {
     ...
     */
    public LoomBuilder addSkill(String name, String description, List<String> tags, String body) {
    ...
    /**
     * 设置自定义 TaskStore 实现。
     * <p>需要 sprinkle-loom-agent-ext 在 classpath。默认使用 {@code FileTaskStore}。
     * 传入的对象必须实现 {@code icu.sprinkle.loom.ext.task.TaskStore} 接口。</p>
     *
     * @param taskStore TaskStore 实现实例
     */
    public LoomBuilder taskStore(Object taskStore) {
     ...
     * <p>传入的对象必须实现 {@code icu.sprinkle.loom.core.memory.MemoryStore} 接口。</p>
     *
     * @param store MemoryStore 实现实例
     */
    public LoomBuilder memoryStore(Object store) {
     ...
     * 需要 sprinkle-loom-mcp 在 classpath。
     */
    public LoomBuilder enableMcp(List<McpServerConfig> servers) {
        if (servers != null) {
     ...
     */
    public LoomBuilder addMcpServer(McpServerConfig server) {
        if (server != null) {
    ...
    public Loom build() {
        ...
        if (!mcpServers.isEmpty()) {
            if (!McpRegistrar.isAvailable()) {
                throw new IllegalStateException(
                        "enableMcp() requires sprinkle-loom-mcp on classpath.");
            }
        ...
        if (enableTodoWrite) {
            registry.register(new icu.sprinkle.loom.tool.builtin.TodoWriteTool());
        ...
        if (enableManualCompact && compactionThreshold > 0) {
            registry.register(new icu.sprinkle.loom.tool.builtin.CompactTool());
        ...
        // MVP8: 长期记忆
        if (memoryStore != null) {
            if (!(memoryStore instanceof icu.sprinkle.loom.core.memory.MemoryStore ms)) {
                throw new IllegalArgumentException(
                        "memoryStore must implement icu.sprinkle.loom.core.memory.MemoryStore, got: "
                                + memoryStore.getClass().getName());
            }
            hooks.add(new icu.sprinkle.loom.core.memory.MemoryEnricherHook(ms));
        ...
        return new Loom(agentLoop, context, sessionManager, resources);
    }
        ...
        if (needsWorkingDir && workingDirectory == null) {
            throw new IllegalStateException(
                    "workingDirectory is required when any of enableFileTools / enableBashTool / "
                            ...
                            + "Call LoomBuilder.workingDirectory(Path).");
        }
    ...
    private LlmProvider resolveLlmProvider() {
        ...
        throw new IllegalStateException(
                "No LlmProviderFactory found for provider '" + targetProvider + "'. "
                        + "Make sure sprinkle-loom-llm-anthropic (or another provider) is on the classpath.");
    }
    ...
    private void discoverAndRegisterTools(ToolRegistry registry) {
        ...
        if (enableFileTools) {
            registry.register(new icu.sprinkle.loom.tool.builtin.ReadFileTool());
            registry.register(new icu.sprinkle.loom.tool.builtin.WriteFileTool());
            registry.register(new icu.sprinkle.loom.tool.builtin.EditFileTool());
            log.info("已启用文件工具（read_file, write_file, edit_file）");
        }
        if (enableBashTool) {
            registry.register(new icu.sprinkle.loom.tool.builtin.BashTool());
        ...
        var toolContext = new icu.sprinkle.loom.tool.ToolContext(workingDirectory);
     ...
     * 创建构建器实例（最低粒度，无任何预设）。
     *
     * @return 新的 LoomBuilder
     */
    public static LoomBuilder create() {
        return new LoomBuilder();
    }
     ...
     * }</pre>
     *
     * @return 预配置的 LoomBuilder（已启用 InMemorySessionStore 支持 chat/resume）
     * @since 0.10.0 (MVP9)
     */
    @icu.sprinkle.loom.api.Experimental("MVP9 引入；预设组合可能根据用户反馈调整")
    public static LoomBuilder chatBot() {
        return new LoomBuilder()
                .sessionStore(new icu.sprinkle.loom.core.session.InMemorySessionStore());
    }
     ...
     * @return 预配置的 LoomBuilder（已 enableCodingTools + enableFileSnapshot）
     * @since 0.10.0 (MVP9)
     */
    @icu.sprinkle.loom.api.Experimental
    public static LoomBuilder codingAgent(java.nio.file.Path workingDirectory) {
        return new LoomBuilder()
                .workingDirectory(workingDirectory)
                .enableCodingTools()
                .enableFileSnapshot();
     ...
     * }</pre>
     *
     * @return 预配置的 LoomBuilder（无内置工具 + 多轮会话支持）
     * @since 0.10.0 (MVP9)
     */
    @icu.sprinkle.loom.api.Experimental
    public static LoomBuilder businessAgent() {
        return new LoomBuilder()
                .sessionStore(new icu.sprinkle.loom.core.session.InMemorySessionStore());
    }
}
